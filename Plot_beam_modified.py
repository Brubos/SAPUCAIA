...
#    image_box.layout().addWidget(plot_canvas)
    ...
    fontsize = 12
    space = 0.06     #distance between graphs
    LTborder = 0.04
    RBborder = 0.15
    X_or_Y = 0.28 
    ...
    if(fitType != 0):
        ...
        # MEAN COORDINATES
        x_mean_fit = np.average(x_axis, weights=x_cut_fit); z_mean_fit = np.average(z_axis, weights=z_cut_fit)      
    ...
    if(fitType != 0):
        output['fit_mean_x'] = x_mean_fit
        output['fit_mean_z'] = z_mean_fit
